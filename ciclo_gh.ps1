...
foreach ($t in $TICKERS) {
    $stockData[$t] = @{ 'prob' = 50; 'conf' = 50; 'sector' = 'General'; 'name' = $t; 'analisis' = '' }
}
...
for ($i = 0; $i -lt 5; $i++) {
    ...
    $top5Html += "<div class=`"t5c`"><div class=`"rk`">#$($i+1)</div><div class=`"tk`">$t</div><div class=`"nm`">$($info['name'])</div><div class=`"pr`" style=`"color:$cc`">`$$($pr['price']) <span style=`"font-size:11px;font-weight:400`">$sg$($pr['pct'])%</span></div><div class=`"an`">$an</div><div class=`"pb`"><div class=`"pbb`"><div class=`"pbf $pc2`" style=`"width:$($info['prob'])%`"></div></div><span class=`"pt`" style=`"color:$cc`">$($info['prob'])%</span></div></div>"
}
$top5Html += "</div></div>"
$HTML += $top5Html
