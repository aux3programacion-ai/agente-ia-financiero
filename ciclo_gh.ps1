...
$HTML += "<div class=`"pf`"><div class=`"pfh`">MI PORTAFOLIO <span>Acciones que sigues - se guarda automaticamente</span></div>"
...
$HTML += "<script>var PD=JSON.parse('$pfJson');var ND=JSON.parse('$newsJson');function agregarAlPortafolio(){var i=document.getElementById('pfInput');var t=i.value.trim().toUpperCase();i.value='';if(!t||!PD[t]){i.placeholder='Ticker no valido (solo 30 monitoreados)';setTimeout(function(){i.placeholder='Ej: NVDA, AAPL, MSFT'},2000);return}var p=JSON.parse(localStorage.getItem('pf')||'[]');if(p.indexOf(t)===-1){p.push(t);localStorage.setItem('pf',JSON.stringify(p))}renderPortfolio()}function eliminarDelPortafolio(t){var p=JSON.parse(localStorage.getItem('pf')||'[]');var idx=p.indexOf(t);if(idx>-1){p.splice(idx,1);localStorage.setItem('pf',JSON.stringify(p))}renderPortfolio()}function renderPortfolio(){var p=JSON.parse(localStorage.getItem('pf')||'[]');var lst=document.getElementById('pfList');var sts=document.getElementById('pfStats');if(p.length===0){lst.innerHTML='<div style=\"color:#4b5563;font-size:12px;grid-column:1/-1;text-align:center;padding:20px\">Agrega tickers arriba para seguir su probabilidad en tiempo real</div>';sts.innerHTML='';return}var html='';var sp=0;var sc=0;var sm=0;var sn=[];p.forEach(function(t){var d=PD[t];if(!d)return;sp+=d.pr;sc+=d.cf;sm+=d.tg;var cc=d.ch>=0?'#00c853':'#ff5252';var sg=d.ch>=0?'+':'';var pbc=d.pr>=65?'pb-h':d.pr>=58?'pb-m':'pb-l';var an=d.an||'Sin analisis';var phl=d.ph>0?('Prec: '+Math.round(d.ph*100)+'%'):'';var ndt=ND[t]||null;var nhtml='';if(ndt){var nsc=ndt.sc>=0?'positivo':'negativo';var nco=ndt.sc>=0.3?'#00c853':ndt.sc<-0.3?'#ff5252':'#ffc107';nhtml='<div class=\"ns\"><div class=\"nl\">Noticia reciente</div><div class=\"nt\">'+ndt.t.substring(0,80)+'</div><div class=\"nsm\" style=\"color:'+nco+'\">Sentimiento: '+nsc+' ('+ndt.sc+')</div></div>'}html+='<div class=\"pfc\"><button class=\"del\" onclick=\"eliminarDelPortafolio(\\''+t+'\\')\">&times;</button><div class=\"tk\">'+t+'</div><div class=\"nm\">'+d.nm+'</div><div class=\"pr\" style=\"color:'+cc+'\">$'+d.p+' <span style=\"font-size:10px;font-weight:400\">'+sg+d.pc+'%</span></div><div class=\"scm\"><span class=\"sb\" style=\"background:'+(d.sc=='Semiconductores'?'#1565c0':d.sc=='Servidores IA'?'#e65100':d.sc=='Software IA'?'#00695c':d.sc=='Ciberseguridad'?'#b71c1c':d.sc=='Consumer Tech'?'#4a148c':'#37474f')+'\">'+d.sc.substring(0,12)+'</span><span style=\"color:#00c853;font-size:11px\">Objetivo 30d: $'+d.tg+'</span></div><div style=\"margin-top:4px\"><div class=\"pb\"><div class=\"pbb\"><div class=\"pbf '+pbc+'\" style=\"width:'+d.pr+'%\"></div></div><span class=\"pt\" style=\"color:'+cc+'\">'+d.pr+'%</span></div></div>'+nhtml+'</div>'});lst.innerHTML=html;var promP=Math.round(sp/p.length);var promC=Math.round(sc/p.length);var promT=p.length>0?Math.round(sm/p.length):0;sts.innerHTML='<div class=\"pv\"><div class=\"l\">Acciones</div><div class=\"v\">'+p.length+'</div></div><div class=\"pv\"><div class=\"l\">Prob. Prom</div><div class=\"v\" style=\"color:'+(promP>=65?'#00c853':promP>=55?'#ffc107':'#ff5252')+'\">'+promP+'%</div></div><div class=\"pv\"><div class=\"l\">Conf. Prom</div><div class=\"v\">'+promC+'%</div></div><div class=\"pv\"><div class=\"l\">Target Prom</div><div class=\"v\" style=\"color:#00c853\">$'+promT+'</div></div>'}renderPortfolio();function filtrarTabla(){var q=document.getElementById('searchInput').value.toUpperCase();var s=document.getElementById('sectorFilter').value;var rows=document.querySelectorAll('tbody tr');var v=0;rows.forEach(function(r){var tk=r.cells[1].textContent.toUpperCase();var nm=r.cells[2].textContent.toUpperCase();var sc=r.cells[3].textContent;var match=q===''||tk.includes(q)||nm.includes(q)||sc.toUpperCase().includes(q);if(s!==''&&sc!==s)match=false;r.style.display=match?'':'none';if(match)v++;});document.getElementById('resultCount').textContent=v+'/'+rows.length;}</script>"
$HTML += "</body></html>"
